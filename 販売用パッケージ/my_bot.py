...
with tab6:
    st.header("📖 使い方ガイド")
    st.caption("このツールの全機能を網羅した完全マニュアルです。各セクションをクリックして開いてください。")

    st.info("このガイドでは、各タブの機能を詳しく解説しています。初めてお使いの方は、まず「アカウント管理」から順番にお読みください。")

    # ==================== Tab1: アカウント管理 ====================
    with st.expander("1. アカウント管理 - Threadsアカウントの登録と管理", expanded=False):
        st.markdown("""
### アカウント管理タブでできること

Threadsアカウントを登録・管理するための基本画面です。ここで登録したアカウントが、投稿・リポスト・引用投稿すべての機能で使われます。

---

#### 新規アカウントの追加

アカウントを追加するには、以下の3つの情報が必要です。

| 項目 | 説明 |
|------|------|
| **アクセストークン** | Meta for Developersで発行するThreads APIのトークン |
| **App ID** | Metaアプリのアプリケーション ID |
| **App Secret** | Metaアプリのシークレットキー |

**手順：**
1. 「Add New Account」セクションを開く
2. 各項目を入力して「保存」をクリック
3. 登録が成功すると、アカウント一覧に表示されます

**過去のアプリ情報を再利用する機能**もあります。一度登録したApp IDとApp Secretは保存されるので、2つ目以降のアカウントを追加するときにワンクリックで入力を省略できます。
...
- **検索**: アカウント名で絞り込みができます
- **ソート**: 「最新追加順」「名前順」「Slot順」で並び替え可能
- **ON/OFF切り替え**: 各アカウント横のトグルで、自動投稿の有効/無効を切り替えます。OFFにすると、そのアカウントの予約投稿はすべて一時停止します
- **ON予約タイマー**: 「今はOFFにしておきたいけど、数時間後に自動でONにしたい」というときに使います。1時間後〜24時間後の範囲で指定できます
- **全員ONボタン**: 登録済みの全アカウントを一括でONにします
- **編集**: トークンやアプリ情報を後から変更できます
- **削除**: 不要になったアカウントを削除します（関連する予約投稿も削除されます）
""")

    # ==================== Tab2: ポストファクトリー ====================
    with st.expander("2. ポストファクトリー - 投稿の作成とスケジュール管理", expanded=False):
        st.markdown("""
### ポストファクトリーの全機能

投稿の作成からスケジュール設定、キュー管理まで、すべてこのタブで行います。最も多機能なタブです。

---

#### 投稿の作成

1. **アカウント選択**: ドロップダウンから投稿に使うアカウントを選びます（検索・ソート対応）
2. **テキスト入力**: 投稿するテキストを入力します。複数行の入力もOKです
3. **画像アップロード**: 画像を添付する場合はここでアップロードします。**複数枚の画像にも対応**しています
4. **「キューに追加」** をクリックすると、投稿キューに登録されます

---

#### スケジュール設定（3つのモード）

投稿のタイミングは、3つのモードから選べます。用途に合わせて使い分けてください。

**1. ランダムスケジュール**
- 時間帯をスライダーで指定（例：8時〜22時）
- 1日の投稿数を設定
- 指定した時間帯の中で、ランダムなタイミングで投稿されます
- 自然な投稿パターンになるので、最もおすすめのモードです

**2. 固定時間スケジュール**
- 「何時何分」を正確に指定して投稿します
- 日数間隔も設定できます（例：2日おきに投稿）
- 毎日決まった時間に投稿したい場合に便利です

**3. ボックス24時間**
- 24時間の各時間帯ごとに投稿数を設定できるテンプレート方式です
- 例：「朝8時台に2件、昼12時台に3件、夜20時台に2件」のように細かく設定可能
- 投稿パターンを完全にコントロールしたい上級者向けです

---

#### 合間機能（アカウント間の投稿間隔調整）

複数アカウントで運用している場合に重要な機能です。

- **最小投稿間隔**を「なし / 1時間 〜 8時間」から選択
- 同じアカウントの投稿が近すぎないように、システムが自動で間隔を調整します
- 例：間隔を「2時間」に設定すると、同一アカウントの投稿は最低2時間の間隔が保たれます
- 凍結リスクを下げるために、**2時間以上の設定を推奨**します

---

#### キュー管理

登録済みの投稿キューに対して、以下の操作ができます。

- **編集**: テキストやスケジュールを後から変更
- **一時停止 / 再開**: 個別の投稿を一時停止、または再開
- **削除**: 不要な投稿キューを削除
- **一括操作**: 「全て再開」「全て停止」「全て削除」ボタンでまとめて操作

---

#### リピート投稿

同じ画像を使いながら、複数パターンのテキストをローテーション投稿する機能です。

- **仕組み**: テキストA → テキストB → テキストC → テキストA ... のように順番に切り替わります
- **テキストごとに表示日数を設定**: 例えば「テキストAを3日間表示 → テキストBを5日間表示」のような設定が可能
- **テキストテンプレートの保存・読み込み**: よく使うテキストパターンを保存しておけば、次回からワンクリックで呼び出せます
- **一時停止 / 再開 / 編集 / 削除**: 通常の投稿キューと同じ操作ができます

---

#### 過去投稿の取得

Threads APIから、アカウントの過去投稿を取得して確認できます。

- **取得件数**: 10件 / 25件 / 50件から選択
- **表示内容**: 投稿テキスト、いいね数、メディア情報（画像・動画）
- どの投稿が反応が良かったかの分析に活用できます

---

#### Watch List（競合監視）

気になる競合アカウントの投稿を自動で監視する機能です。

- 競合アカウントを登録すると、**最新の投稿を自動取得**します
- 投稿に含まれる**画像も自動ダウンロード**されます
- 競合の投稿パターンやトレンドを把握するのに便利です
""")

    # ==================== Tab3: 投稿モニター ====================
    with st.expander("3. 投稿モニター - 投稿状況のリアルタイム監視", expanded=False):
        st.markdown("""
### 投稿モニターの全機能

予約投稿の実行状況をリアルタイムで確認・管理する画面です。毎日チェックすることをおすすめします。

---

#### 今日の統計ダッシュボード

画面上部に、本日の投稿状況がひと目でわかるダッシュボードが表示されます。

| 項目 | 内容 |
|------|------|
| **成功数** | 本日正常に投稿されたた件数 |
| **エラー数** | 投稿に失敗した件数 |
| **成功率** | 成功数 / (成功数+エラー数) のパーセンテージ |
| **稼働アカウント** | 現在ONになっているアカウント数 |

---

#### 各種チェック機能

- **凍結確認ボタン**: 各アカウントがThreadsで凍結されていないかをAPIで確認します。凍結が検出された場合はアラートが表示されます
- **固定投稿チェック**: 固定投稿が正しく設定されているか確認します
- **バズチェック**: いいね数が一定以上の投稿を検出して通知します。バズの兆候を見逃しません
- **取りこぼし検出**: 予定時刻から2時間以上遅延している投稿があれば警告を表示します。APIエラーやシステムの問題を早期発見できます

---

#### エラー詳細と自動修復

- 投稿エラーが発生した場合、**エラーの詳細内容**が表示されます
- トークン期限切れなど、自動で修復できるエラーは**自動修復ログ**に記録されます
- 修復できないエラーの場合は、手動対応が必要な旨のメッセージが表示されます
""")

    # ==================== Tab4: リポスト ====================
    with st.expander("4. リポスト - 他の投稿をワンクリックで再投稿", expanded=False):
        st.markdown("""
### リポスト機能の全体像

他のユーザーの投稿をリポスト（再投稿）する機能です。複数アカウントで一括リポストもできます。

---

#### ボックス作成（アカウントのグループ化）

まず、リポストに使うアカウントをグループにまとめた「ボックス」を作成します。

- ボックスに名前をつけて、使いたいアカウントを選択
- 一度作成すれば繰り返し使えます
- 用途別にボックスを作ると便利です（例：「メインアカウント用」「サブアカウント用」）

---

#### 即時リポスト

今すぐリポストしたい場合はこちらを使います。

1. リポストしたい投稿のURLを入力
2. 使用するボックスを選択
3. 「実行」をクリック → ボックス内の全アカウントで一括リポスト
...
予約したリポストは**キャンセル**も可能です。

---

#### 実行履歴

過去に実行したリポストの履歴が確認できます。いつ、どのアカウントで、どの投稿をリポストしたかが一覧で見られます。
""")

    # ==================== Tab5: 引用投稿 ====================
    with st.expander("5. 引用投稿 - コメント付きで他の投稿をシェア", expanded=False):
        st.markdown("""
### 引用投稿の全機能

他のユーザーの投稿を引用して、自分のコメント付きで投稿する機能です。リポストとの違いは「自分のコメントを添えられる」点です。

---

#### ボックス作成

リポストと同様に、アカウントをグループ化した「ボックス」を作成します。引用投稿専用のボックスを作ることもできますし、リポストのボックスとは別に管理されます。

---

#### 一括引用投稿

ボックスに登録された全アカウントで同じ投稿を引用します。

1. 引用元の投稿URLを入力
2. 引用コメント（自分のテキスト）を入力
3. ボックスを選択
4. 「投稿」をクリック → ボックス内の全アカウントで一括引用投稿

---

#### 単体引用投稿

1つのアカウントだけで引用投稿したい場合はこちらを使います。
